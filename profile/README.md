
<div align="center">

# ALS Accelerator Physics Group

### Advanced Light Source • Lawrence Berkeley National Laboratory

[![Visit ALS](https://img.shields.io/badge/Visit-ALS%20Website-blue?style=for-the-badge)](https://als.lbl.gov)
[![Berkeley Lab](https://img.shields.io/badge/Berkeley-Lab-green?style=for-the-badge)](https://www.lbl.gov)

</div>


## 💡 The Advanced Light Source

The **Advanced Light Source (ALS)** at [Lawrence Berkeley National Laboratory](https://www.lbl.gov) is a U.S. Department of Energy user facility that provides researchers worldwide with **bright beams of ultraviolet and x-ray light** for studies in physics, materials science, chemistry, and biology.  

Operating a **1.9 GeV electron storage ring** in continuous **top-off mode**, the ALS delivers stable, high-brightness light to nearly **43 beamlines**.  

The upcoming [ALS-U upgrade]((https://als.lbl.gov/als-u/overview/)) will boost brightness by more than 100×, enabling experiments with **nanometer spatial** and **nanosecond temporal** resolution.


## 🚀 Accelerator Physics at the ALS

The **Accelerator Physics Group** develops and operates the accelerator systems that enable the Advanced Light Source to deliver **stable, high-brightness beams** for user experiments.  

We advance **lattice design, insertion devices, beam dynamics, and feedback systems** to ensure **reliable, high-uptime operation** of the light source.  

Building on decades of accelerator expertise, our current efforts focus on **AI-assisted control, data-driven optimization, and digital-twin modeling** to prepare for the **ALS-U era** and the next generation of light-source performance.



## 🧩 Our Projects

| Project | Description |
|----------|--------------|
| 🦅 **Osprey Framework** — [Repo](https://github.com/als-apg/osprey) · [Paper](https://arxiv.org/abs/2508.15066) · [Docs](https://als-apg.github.io/osprey/) | Orchestrated AI framework for multi-agent scientific workflows |
| 🤖 **ALS Assistant** — [Repo](https://github.com/als-apg/als_assistant) · [Paper](https://arxiv.org/abs/2509.17255) | Accelerator-aware AI assistant for real-time operations and research |
| 🧠 **AccPhysBERT** — [Model](https://huggingface.co/collections/thellert/accphysbert) · [Paper](https://doi.org/10.1103/PhysRevAccelBeams.28.044601) | Domain-specific text embedding model for accelerator physics literature |
| 🧠 **PhysBERT** — [Model](https://huggingface.co/collections/thellert/physbert) · [Paper](https://doi.org/10.1063/5.0238090) | Domain-specific text embedding model for physics scientific literature |


## 📚 Resources

### About the ALS
- [ALS Official Website](https://als.lbl.gov)
- [ALS-U Project](https://als.lbl.gov/als-u/overview/)
- [ALS Computing Group](https://als.lbl.gov/computing-site/)
- [ALS Operating Schedule](https://als.lbl.gov/operating-schedule/)

### Lawrence Berkeley National Laboratory
- [Berkeley Lab Homepage](https://www.lbl.gov)
- [Computing Sciences Area](https://crd.lbl.gov)
- [Energy Science Area](https://energysciences.lbl.gov)

## 📄 License

Our repositories are released under open-source licenses.  
Please refer to individual project directories for specific license details.

---

<div align="center">

*Funded by the U.S. Department of Energy, Office of Science.*  

[![DOE](https://img.shields.io/badge/Funded%20by-U.S.%20DOE-gold?style=flat-square)](https://www.energy.gov/science/office-science)

---

© 2025 Lawrence Berkeley National Laboratory.  
Operated by the University of California for the U.S. Department of Energy, Office of Science.

</div>
